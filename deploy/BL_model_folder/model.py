...
class LaneGeometryDetector:
    """Detect the left-lane corridor from white track markings."""
    ...
    def estimate(self, bgr_frame: np.ndarray) -> LaneGeometry:
        ...
        lane_width = float(max(0.22, min(0.55, lane_width)))
    ...
    def _estimate_band(
        ...
        if left_x is not None and centre_x is not None and centre_x > left_x + 0.12:
            ...
            target_x = float(left_x + lane_width * LANE_TARGET_POSITION)
        elif left_x is not None:
            lane_width = self.last_width_frac
            target_x = float(left_x + lane_width * LANE_TARGET_POSITION)
        elif centre_x is not None:
            lane_width = self.last_width_frac
            target_x = float(centre_x - lane_width * (1.0 - LANE_TARGET_POSITION))
        else:
            return None
