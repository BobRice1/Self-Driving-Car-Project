...
# OpenCV arrow sign detector settings. This copy intentionally does not use the
# TFLite arrow classifier; it finds small blue sign circles and estimates arrow
# direction from white-pixel centroid and edge balance inside the sign crop.
ARROW_CLASSES = ["none", "left", "right"]
ARROW_INPUT_MODE = "opencv_sign"
ARROW_USE_FULL_FRAME = False
ARROW_ROI = (0, 0, 320, 130)
ARROW_BLUE_MIN_AREA = 20
ARROW_BLUE_MAX_AREA = 1800
ARROW_BLUE_MAX_AREA_RATIO = 0.035
ARROW_BLUE_MIN_AREA_RATIO = 0.00015
ARROW_BLUE_PAD = 12
ARROW_BLUE_CORRIDOR_X_MIN = 0.15
ARROW_BLUE_CORRIDOR_X_MAX = 0.85
ARROW_BLUE_CORRIDOR_Y_MIN = 0.00
ARROW_BLUE_CORRIDOR_Y_MAX = 0.70
ARROW_BLUE_HSV_LOWER = np.array([85, 45, 40])
ARROW_BLUE_HSV_UPPER = np.array([140, 255, 255])
ARROW_STRONG_BLUE_HSV_LOWER = np.array([85, 120, 60])
ARROW_STRONG_BLUE_HSV_UPPER = np.array([140, 255, 255])
ARROW_MAX_DIM_RATIO = 0.28
ARROW_BOTTOM_ZONE_RATIO = 0.92
ARROW_EDGE_MARGIN_RATIO = 0.03
ARROW_MIN_ASPECT_NEAR_SQUARE = 0.45
ARROW_MIN_CIRCULARITY = 0.45
ARROW_MIN_ENCLOSING_FILL = 0.30
ARROW_MIN_BLUE_RATIO = 0.18
ARROW_MIN_EDGE_DENSITY = 0.03
ARROW_HOUGH_DP = 1.2
ARROW_HOUGH_PARAM1 = 120
ARROW_HOUGH_PARAM2 = 18
ARROW_HOUGH_MIN_RADIUS_RATIO = 0.035
ARROW_HOUGH_MAX_RADIUS_RATIO = 0.16
ARROW_WHITE_HSV_LOWER = np.array([0, 0, 130])
ARROW_WHITE_HSV_UPPER = np.array([180, 100, 255])
ARROW_DIRECTION_MIN_SIZE = 8
ARROW_DIRECTION_WHITE_MIN_PX = 3
ARROW_DIRECTION_CENTROID_THRESH = 0.08
ARROW_DIRECTION_SOBEL_LEFT_RATIO = 1.25
ARROW_DIRECTION_SOBEL_RIGHT_RATIO = 0.75
SIGN_IMMINENT_PX_LEFT = 34.0
SIGN_IMMINENT_PX_RIGHT = 34.0
ARROW_FLIP_HORIZONTAL = False
ARROW_SWAP_LEFT_RIGHT = False
ARROW_CONFIDENCE_THRESHOLD = 0.50
...
class DebugStream:
    ...
    def _extract_arrow_image(image: np.ndarray) -> np.ndarray:
        if ARROW_INPUT_MODE.lower() == "opencv_sign":
            arrow_image, _ = Model._arrow_input_image(image)
            return arrow_image.copy()
...
class Model:
    def __init__(self) -> None:
        ...
        print(
            ...
            f"arrow=opencv, obstacle={self.obstacle.available}, "
    ...
    def _update_arrow(self, image: np.ndarray) -> None:
        arrow, confidence, class_probs, arrow_box, distance = self._detect_best_arrow_sign(image)
        ...
        if ARROW_DEBUG_PROBS:
            ...
            print(
                f"[arrow-opencv] raw={arrow}:{confidence:.3f} probs {prob_text} "
                f"distance={distance} threshold={ARROW_CONFIDENCE_THRESHOLD:.2f} box={arrow_box}"
            )
    ...
    def _arrow_input_image(image: np.ndarray) -> tuple[np.ndarray, Optional[tuple[float, float, float, float]]]:
        _, _, _, box, _ = Model._detect_best_arrow_sign(image)
        if box is not None:
            h, w = image.shape[:2]
            x1 = max(0, min(w, int(box[0] * w)))
            y1 = max(0, min(h, int(box[1] * h)))
    ...
    @staticmethod
    def _detect_best_arrow_sign(image: np.ndarray) -> tuple[str, float, dict[str, float], Optional[tuple[float, float, float, float]], str]:
        h, w = image.shape[:2]
        if h <= 0 or w <= 0:
            probs = {"none": 1.0, "left": 0.0, "right": 0.0}
            return "none", 1.0, probs, None, "none"

        hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
        min_area = max(float(ARROW_BLUE_MIN_AREA), float(h * w) * ARROW_BLUE_MIN_AREA_RATIO)
        max_area = float(h * w) * ARROW_BLUE_MAX_AREA_RATIO
        max_dim = int(ARROW_MAX_DIM_RATIO * min(h, w))
        edge_margin = max(5, int(ARROW_EDGE_MARGIN_RATIO * w))

        blue = Model._blue_validation_mask(hsv)
        candidate_mask = Model._circle_candidate_mask(image, hsv, blue)
        ...
        for contour in contours:
            ...
            if cx < edge_margin or cx > w - edge_margin:
                continue
            if cy > h * ARROW_BOTTOM_ZONE_RATIO:
                continue
            squareness = min(bw, bh) / max(max(bw, bh), 1)
            if squareness < ARROW_MIN_ASPECT_NEAR_SQUARE:
                continue
            if max(bw, bh) > max_dim:
                continue
            perimeter = cv2.arcLength(contour, True)
            circularity = (4.0 * np.pi * area) / max(perimeter * perimeter, 1e-6)
            if circularity < ARROW_MIN_CIRCULARITY:
                continue
            (_, _), radius = cv2.minEnclosingCircle(contour)
            circle_area = np.pi * radius * radius
            enclosing_fill = area / max(circle_area, 1.0)
            if enclosing_fill < ARROW_MIN_ENCLOSING_FILL:
                continue

            contour_mask = np.zeros((h, w), dtype=np.uint8)
            cv2.drawContours(contour_mask, [contour], -1, 255, -1)
            mean_v = cv2.mean(hsv[:, :, 2], mask=contour_mask)[0]
            mean_s = cv2.mean(hsv[:, :, 1], mask=contour_mask)[0]
            if mean_v < 75 or mean_s < 90:
                continue
            blue_px = cv2.countNonZero(cv2.bitwise_and(blue, contour_mask))
            blue_ratio = blue_px / max(float(cv2.countNonZero(contour_mask)), 1.0)
            if blue_ratio < ARROW_MIN_BLUE_RATIO:
                continue
            ...
            direction = Model._read_arrow_orientation(roi)
            if direction not in ("left", "right"):
                continue

            diameter = float(radius * 2.0)
            distance = Model._classify_sign_range(diameter, direction)
            confidence = min(
                1.0,
                0.45
                + 0.25 * min(1.0, area / max(max_area, 1.0))
                + 0.20 * min(1.0, blue_ratio)
                + 0.10 * min(1.0, circularity)
                + (0.10 if distance == "imminent" else 0.0),
            )
            detections.append({
                "direction": direction,
                "confidence": confidence,
                "box": (rx1 / max(w, 1), ry1 / max(h, 1), rx2 / max(w, 1), ry2 / max(h, 1)),
                "distance": distance,
                "area": area,
                "diameter": diameter,
            })

        detections.extend(Model._hough_arrow_signs(image, hsv, blue, min_area, max_area, max_dim, edge_margin))
        ...
        probs = {"none": 1.0 - confidence, "left": 0.0, "right": 0.0}
        probs[arrow] = confidence
        return arrow, confidence, probs, best["box"], str(best["distance"])

    @staticmethod
    def _blue_validation_mask(hsv: np.ndarray) -> np.ndarray:
        small_open = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (3, 3))
        small_close = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5))
        large_close = cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (9, 9))

        broad_blue = cv2.inRange(hsv, ARROW_BLUE_HSV_LOWER, ARROW_BLUE_HSV_UPPER)
        broad_blue = cv2.morphologyEx(broad_blue, cv2.MORPH_OPEN, small_open)
        broad_blue = cv2.morphologyEx(broad_blue, cv2.MORPH_CLOSE, small_close)

        saturated_blue = cv2.inRange(hsv, ARROW_STRONG_BLUE_HSV_LOWER, ARROW_STRONG_BLUE_HSV_UPPER)
        saturated_blue = cv2.morphologyEx(saturated_blue, cv2.MORPH_OPEN, small_open)
        saturated_blue = cv2.morphologyEx(saturated_blue, cv2.MORPH_CLOSE, large_close)
        return cv2.bitwise_or(broad_blue, saturated_blue)

    @staticmethod
    def _circle_candidate_mask(image: np.ndarray, hsv: np.ndarray, blue: np.ndarray) -> np.ndarray:
        ...
        edges = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (5, 5)))

        # Add the blue validation mask so weak sign edges are still considered,
        # but final acceptance is based on shape first, then blue ratio.
        candidates = cv2.bitwise_or(edges, blue)
        candidates = cv2.morphologyEx(candidates, cv2.MORPH_CLOSE, cv2.getStructuringElement(cv2.MORPH_ELLIPSE, (7, 7)))
        ...
        corridor = np.zeros_like(candidates)
        x1 = int(w * ARROW_BLUE_CORRIDOR_X_MIN)
        x2 = int(w * ARROW_BLUE_CORRIDOR_X_MAX)
        y1 = int(h * ARROW_BLUE_CORRIDOR_Y_MIN)
        y2 = int(h * ARROW_BLUE_CORRIDOR_Y_MAX)
        corridor[max(0, y1):min(h, y2), max(0, x1):min(w, x2)] = 255
        return cv2.bitwise_and(candidates, corridor)

    @staticmethod
    def _hough_arrow_signs(
        image: np.ndarray,
        hsv: np.ndarray,
        blue: np.ndarray,
        min_area: float,
        ...
        min_radius = max(4, int(min(h, w) * ARROW_HOUGH_MIN_RADIUS_RATIO))
        max_radius = max(min_radius + 1, int(min(h, w) * ARROW_HOUGH_MAX_RADIUS_RATIO))
        circles = cv2.HoughCircles(
            gray,
            cv2.HOUGH_GRADIENT,
            dp=ARROW_HOUGH_DP,
            minDist=max(12, min_radius * 2),
            param1=ARROW_HOUGH_PARAM1,
            param2=ARROW_HOUGH_PARAM2,
        ...
        for cx, cy, radius in np.round(circles[0]).astype(int):
            ...
            blue_px = cv2.countNonZero(cv2.bitwise_and(blue, circle_mask))
            blue_ratio = blue_px / max(float(cv2.countNonZero(circle_mask)), 1.0)
            if blue_ratio < ARROW_MIN_BLUE_RATIO:
                continue
            mean_v = cv2.mean(hsv[:, :, 2], mask=circle_mask)[0]
            ...
            confidence = min(
                1.0,
                0.50
                + 0.20 * min(1.0, area / max(max_area, 1.0))
                + 0.25 * min(1.0, blue_ratio)
                + (0.10 if distance == "imminent" else 0.0),
            )
            detections.append({
                "direction": direction,
    ...
    @staticmethod
    def _classify_sign_range(sign_diameter: float, arrow_direction: str) -> str:
        threshold = SIGN_IMMINENT_PX_RIGHT if arrow_direction == "right" else SIGN_IMMINENT_PX_LEFT
        return "imminent" if sign_diameter >= threshold else "approaching"

    @staticmethod
    def _read_arrow_orientation(roi: np.ndarray) -> str:
        bh, bw = roi.shape[:2]
        if min(bw, bh) < ARROW_DIRECTION_MIN_SIZE:
            return "unknown"
        ...
        white_mask = cv2.inRange(hsv_roi, ARROW_WHITE_HSV_LOWER, ARROW_WHITE_HSV_UPPER)
        white_in_circle = cv2.bitwise_and(white_mask, circle_mask)
        white_px = cv2.countNonZero(white_in_circle)

        centroid_vote = 0
        if white_px >= ARROW_DIRECTION_WHITE_MIN_PX:
            ys, xs = np.where(white_in_circle > 0)
            offset_ratio = (np.mean(xs) - bw / 2.0) / max(bw / 2.0, 1.0)
            if offset_ratio < -ARROW_DIRECTION_CENTROID_THRESH:
                centroid_vote = -1
            elif offset_ratio > ARROW_DIRECTION_CENTROID_THRESH:
                centroid_vote = 1
        ...
        if left_edges + right_edges > 0:
            ...
            if ratio > ARROW_DIRECTION_SOBEL_LEFT_RATIO:
                sobel_vote = -1
            elif ratio < ARROW_DIRECTION_SOBEL_RIGHT_RATIO:
                sobel_vote = 1
